...
*Regime adaptation* (Exp.~22). GP's ability to adapt to market regimes is well documented: #cite(<allen1999using>, form: "prose") found that GP rules perform differently across market states, and #cite(<dempster2001realtime>, form: "prose") built an explicit real-time adaptive GP trading system. Our contribution is a structural analysis of *how* trees differ---bear markets produce the largest, most conditional trees (mean size~12.9), while sideways markets produce simple trend-following trees (size~3.5)---on modern BTC data. In a single-split experiment, the training data is dominated by one regime (bull in 2014--2019), so seeds that initialise near trend-following structures are favoured.

*Structural predictability* (Exp.~23). That structural complexity predicts overfitting better than tree size is established: #cite(<vanneschi2010measuring>, form: "prose") showed that functional complexity, not just size, predicts overfitting; #cite(<goncalves2015model>, form: "prose") confirmed this across multiple benchmarks. Our contribution is identifying nesting ratio as the dominant predictor on this BTC task ($r = 0.81$) and showing that parsimony pressure at $lambda = 500$ controls tree shape rather than merely limiting size.
