...
#let abstract = [
  Genetic programming (GP) exhibits high variance on financial forecasting tasks: some runs find profitable strategies while others converge to trivial or unprofitable ones. The source of this variance is not well understood empirically. We instrument GP to log every parent-offspring pair across 10 independent seeds (13,245 pairs total) and find that parent-offspring fitness correlation declines from $r = 0.51$ at generation~0 to $r = -0.323$ at generation~10---crossover shifts from preserving advantage to systematically destroying it. This epistasis effect is a measurable mechanism that contributes to GP's instability. Two supplementary findings contextualise the result: (1)~GP adapts tree structure to market regime---bear markets demand complex conditional trees (mean size~12.9) while sideways markets need simple trend indicators (size~3.5); (2)~tree nesting ratio predicts train-test divergence ($r = 0.81$, $p < 10^{-16}$), showing that overfitting has structural signatures visible before testing. Together, these results frame GP's variance not as an irreducible noise floor but as a set of mechanisms that can be monitored and controlled.
]
...
Genetic programming (GP) @koza1992genetic evolves program trees to discover strategy structure automatically. On financial forecasting tasks, it is frequently observed that GP produces high variance across random seeds---some runs find profitable strategies while others converge to trivial or unprofitable ones. The source of this variance is typically attributed to the size and complexity of the search space, but the specific mechanism has not been measured directly.

We address this gap by instrumenting GP to record every parent-offspring pair and measuring how the correlation between parent fitness and offspring fitness evolves over generations. When GP swaps a subtree from one parent into another, the semantic value of that subtree depends on its original context---a phenomenon known as _epistasis_ @oreilly1995building @krawiec2013semantic. A subtree that is profitable in one parent may be harmful in another, because its surrounding nodes determine how its output is interpreted. As generations progress and the population diversifies, the probability that a transplanted subtree retains its original meaning decreases. Crossover becomes increasingly destructive.

This leads to three concrete questions. First, can we measure the temporal dynamics of epistasis in GP crossover? Second, does GP's structural search adapt to different market regimes, and if so, how? Third, can structural properties of evolved trees predict overfitting before testing?

We address these questions through three empirical studies on a Bitcoin trading task:
...
We have shown that GP's variance on a Bitcoin trading task has three mechanistic sources: epistasis in crossover, regime-dependent structural demands, and structural signatures of overfitting. The central finding---that parent-offspring fitness correlation declines from $r = 0.51$ to $r = -0.323$ by generation~10---is direct empirical evidence that GP's primary search operator becomes systematically destructive as evolution proceeds. This is an intrinsic property of context-dependent subtree crossover.
