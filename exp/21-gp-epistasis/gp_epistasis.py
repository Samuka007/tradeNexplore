"""Genetic Programming for tree-structured trading rules — epistasis-instrumented.

Self-contained copy of tiny_bot/gp.py with added parent-offspring logging
for the epistasis experiment.  Does not import from tiny_bot.gp.
"""
...
# ── helpers (inlined from tiny_bot/gp.py) ────────────────────────────────────
...
class GP:
    """Tree-based Genetic Programming with epistasis instrumentation."""
    ...
    def __init__(
        ...
        self.arity = {"+": 2, "-": 2, "*": 2, "/": 2, "ABS": 1, "MAX": 2, "MIN": 2,
                      ">": 2, "<": 2, "AND": 2, "IF": 3}
        self.max_depth = 5
        # Epistasis instrumentation
    ...
    def evaluate(self, tree: GPNode, prices: np.ndarray, continuous: bool = False,
                 log_signals: bool = False) -> np.ndarray:
        ...
        log_signals=True: return raw signal vector before thresholding (for semantic hashing).
        """
        n = len(prices)
    ...
    def _select(self, pop: list[GPNode], fit: list[float]) -> tuple[GPNode, int]:
        """Tournament selection returning (node, population_index)."""
        idxs = np.random.choice(len(pop), size=3, replace=False)
        best_idx = idxs[np.argmax([fit[i] for i in idxs])]
        return pop[best_idx], best_idx

    def optimize(self, fitness_fn, n_workers: int = 1):
        """Return dict with keys: best, fitness, history.

        Side-effect: populates self.parent_offspring_log with per-crossover records.
        Each record is a dict with keys:
          generation, offspring_raw_fit, parent1_raw_fit, parent2_raw_fit, parent_mean_raw_fit.
        ...
        for gen_idx in range(self.generations):
            ...
            # Track offspring whose parents we recorded during crossover
            pending_offspring: list[tuple[int, float, float]] = []

            while len(new) < self.pop_size:
                r = np.random.rand()
                if r < 0.9:
                    # Crossover — log parent raw fitness
                    (p1, i1), (p2, i2) = self._select(pop, fit), self._select(pop, fit)
                    p1_raw = raw_fit[i1]
                    p2_raw = raw_fit[i2]
                    offspring = self._crossover(p1, p2)
                    new.append(offspring)
                    pending_offspring.append((len(new) - 1, p1_raw, p2_raw))
                elif r < 0.95:
                    # Mutation
                    (p, _) = self._select(pop, fit)
            ...
            # Resolve pending offspring records now that we have raw_fit for the new pop
            parent_mean_offspring_raw = {}
            for oidx, p1r, p2r in pending_offspring:
                offspring_raw = raw_fit[oidx]
                parent_mean = (p1r + p2r) / 2.0
                self.parent_offspring_log.append({
                    "generation": gen_idx,
                    "offspring_raw_fit": float(offspring_raw),
                    "parent1_raw_fit": float(p1r),
                    "parent2_raw_fit": float(p2r),
                    "parent_mean_raw_fit": float(parent_mean),
                })
